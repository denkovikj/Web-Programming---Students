    ...
    public List<Student> filter(Long courseId, Integer yearsOfStudying) {
        if(courseId == null && yearsOfStudying == null){
           return studentRepository.findAll();
            ...
            LocalDate dateBefore = LocalDate.now().minusYears(yearsOfStudying);
            return studentRepository.findByCoursesAndEnrollmentDateBefore(course,dateBefore);
        } else if(courseId != null){
            Course course = courseRepository.findById(courseId).orElseThrow(InvalidCourseIdException::new);
            return studentRepository.findByCourses(course);
        }else {
            LocalDate dateBefore = LocalDate.now().minusYears(yearsOfStudying);
            return studentRepository.findByEnrollmentDateBefore(dateBefore);
        }
    }
}
